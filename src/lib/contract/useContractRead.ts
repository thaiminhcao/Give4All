...
export const useContractCall = (
  functionName: string,
  contractAddress: string = mainContract.address,
  abi: string = 'main',
  args?: Array<any>,
  watch?: boolean,
  ...
  if (abi == 'project') abiJson = projectContract.abi;
  const resp = useContractRead({
    ...
    account: contractAddress as `0x${string}`, // Add the 'account' property with a value of undefined
  });
  return resp;
};
